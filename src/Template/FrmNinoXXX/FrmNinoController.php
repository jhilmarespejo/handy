...
class FrmNinoController extends AppController {
	...
	public function add( $id_nino = null ){
        ...
        if ($this->request->is('post')) {
           ...
           // $this->request->data['consideraciones']['fecha_hora'] = date('Y-m-d H:i:s');

//pr($this->request->data['datosnino']['id_nino']);
    ...
    public function control( $id_nino = null ){
         ...
         if( $id_nino ) {
           ...
           /* pr($datosnino);
            ...
            $this->loadmodel('Consideraciones');
            $consideraciones = $this->Consideraciones->find()->select(['id_nino', 'usuario', 'fecha_hora'])->order(['usuario', 'fecha_hora'])->toArray();
    ...
    public function edit( $id_nino = null ){
        ...
        if( $id_nino || !isset($this->request->data['data']['1'])  ) {
            ...
            $this->set(['cims' => $cims, 'nino' => $datosnino, 'vacuna'=>$vacunas, 'sitFamiliar'=>$situacionfamiliar,'familiares' => $familiares, 'cVivienda'=>$caracteristicasvivienda]);
        }

        if ($this->request->is(['post', 'put']) && !isset($this->request->data['data']['1'])) {
            $idiomas ='';
            if( !empty($this->request->data['situacionfamiliar']['idiomas']) ){
                foreach ($this->request->data['situacionfamiliar']['idiomas'] as $key => $value) {
                    $idiomas .=  $value.',';
                }
                $this->request->data['situacionfamiliar']['idiomas'] = $idiomas;
            }    
            ...
            if( isset($this->request->data['familiares']) ){
                foreach ( $this->request->data['familiares'] as $key => $fam ) {
                    if( !isset($fam['id']) ){
                        $fam['id_nino'] = $this->request->data['datosnino']['id_nino'];
           ...
           if( $this->Datosnino->save($datosnino) && $this->Situacionfamiliar->save($situacionfamiliar) && $this->Caracteristicasvivienda->save($caracteristicasvivienda) && $this->Vacunas->save($vacunas) ) {
                $success = true;
            } else {
                    $success = false;
            }
        ...
        } // $this->request->is(['post', 'put']) && !isset($th
    ...
    public function find(){
        ...
        if( isset($this->request->data['nombres']) ) {
            ...
            $datosnino = $this->Datosnino->find()->select(['id_nino','paterno','materno','nombres'])->where(["nombres LIKE" => $nombres, " causas_retiro is Null"])->order(['nombres', 'sexo']);
        }   
