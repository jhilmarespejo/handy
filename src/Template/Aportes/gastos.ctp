...
<h2 class="title2 text-center">Control mensual de gastos de aportes</h2>
...
<div class="board col-xs-12 col-sm-12 col-md-12 col-lg-12">
	<div class="form-group col-xs-12 col-sm-4 col-md-4 col-lg-4" id='periodo'>
		<?php echo $this->Form->input('periodo', ['minYear' => 2014,'maxYear' => date('Y'), 'day'=>false, 'label' => ['text' => 'Período'], 'class'=>'form-control', 'type' => 'date'] ); 
	...
	<div id="cims"></div>
	<!-- <?php echo $this->Form->input('periodo', ['label' => ['text' => ''], 'class'=>'form-control', 'type' => 'text'] ); ?> -->
</div>
...
jQuery(document).ready(function($) {
	$('#id-cim, div#periodo div select').change(function(event) {
		...
		//var action = 'gastos';
		var data = [$('#id-cim').val(), year, month, 'gastos' ];
		$.ajax({
            type: "POST",
            url: "get",
            data: {data},
