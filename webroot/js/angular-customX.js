...
$postModule.controller('PostController',function($scope, $http){
	...
	//var url = base_path+'ajax.php';/*http://localhost/cake/vendor/ajax.php*/


	$scope.init = function(){
		url = 'http://localhost/cake/Employees/getUsers';
	    $http({
	      method: 'post',
	      url: url,
	...
	$scope.saveUser = function(){
		url = 'http://localhost/cake/Employees/saveUser';
		console.log($scope.tempUser);
	    $http({
	      method: 'post',
	      url: url,
	      data: $.param({'user' : $scope.tempUser, 'type' : 'saveUser' }),
	      headers: {'Content-Type': 'application/x-www-form-urlencoded'}
	    }).
	    success(function(data, status, headers, config) {
	    	if(data.success){
